...
public class GameManager {
    ...
    // --- ALTERNATIVA À FLAG: Memória da Posição ---
    private int posicaoAntesDoTurno = -1;

    // --- GETTER NECESSÁRIO PARA O ABYSS ---
        ...
        if (abyssesAndTools != null) {
            ...
            for (String[] dados : abyssesAndTools) {
                ...
                try {
                    ...
                    if (tipoStr.equals("0")) { if (idItem < 0 || idItem > 9){return false;} }
                    else if (tipoStr.equals("1")) { if (idItem < 0 || idItem > 5){return false;} }
                    else { return false; }
    ...
    public String getImagePng(int nrSquare){ return null; }
    ...
    public boolean moveCurrentPlayer(int nrSpaces) {
        ...
        // 1. CAPTURAR POSIÇÃO INICIAL (A "Alternativa" à flag)
        // Guardamos onde o jogador está antes de tentarmos mexê-lo.
        this.posicaoAntesDoTurno = p.getPosicao();

        // 2. Validações de Linguagem (DEVEM SER FEITAS ANTES DO STUCK CHECK)
        ...
        // 3. Verificar se está morto/derrotado
        ...
        if (p.getTurnosPreso() > 0) {
            ...
            atual = (atual + 1) % numJogadores;
            rondas++;
            // Retorna TRUE. Como ele não se mexeu,
            // p.getPosicao() == posicaoAntesDoTurno será VERDADEIRO no react().
            return true;
        }
    ...
    public boolean saveGame(File file) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(file))) {
            ...
            for (Player p : listaPlayers) { writer.println(p.getDataForSave()); }
    ...
    public void loadGame(File file) throws FileNotFoundException, InvalidFileException {
        ...
        try (Scanner scanner = new Scanner(file)) {
            ...
            try {
                ...
                if (tamanhoTabuleiro <= 0 || numJogadores < 0) { throw new InvalidFileException("Dados globais inválidos"); }
                this.currentPlayer = new int[numJogadores];
            } catch (NumberFormatException e) { throw new InvalidFileException("Erro no cabeçalho"); }

            if (!scanner.hasNextLine()){throw new InvalidFileException("Falta nº jogadores");}
            int qtdPlayers;
            try { qtdPlayers = Integer.parseInt(scanner.nextLine()); } catch (NumberFormatException e) { throw new InvalidFileException("Erro numérico qtd players"); }
            ...
            for (int i = 0; i < numJogadores; i++) {
                if (i < idJogadores.size()) { currentPlayer[i] = idJogadores.get(i); }
            }

            if (!scanner.hasNextLine()){throw new InvalidFileException("Falta nº elementos tabuleiro");}
            int qtdElementos;
            try { qtdElementos = Integer.parseInt(scanner.nextLine()); } catch (NumberFormatException e) { throw new InvalidFileException("Erro numérico qtd elementos"); }
        ...
        } catch (FileNotFoundException | InvalidFileException e) { throw e; } catch (Exception e) { throw new InvalidFileException("Erro desconhecido: " + e.getMessage()); }
    }
    ...
    private void restaurarJogador(String linha) throws InvalidFileException {
        ...
        try {
            ...
            idJogadores.add(id);
        } catch (NumberFormatException e) { throw new InvalidFileException("Erro numérico no jogador"); }
    }
    ...
    private void restaurarElementoTabuleiro(String linha) throws InvalidFileException {
        ...
        try {
            ...
            String titulo = p[3];
            BoardElement elemento;
            if (tipoId == 1) { elemento = new Tool(id, titulo); } else { elemento = new Abyss(id, pos, titulo, ""); }
            tabuleiro.put(pos, elemento);
        } catch (NumberFormatException e) { throw new InvalidFileException("Erro numérico no tabuleiro"); }
    }

    // --- REAÇÃO BASEADA NA COMPARAÇÃO DE POSIÇÕES ---
    public String reactToAbyssOrTool() {
        ...
        // Se a posição não mudou desde o início do turno, não faz nada
        // (Significa que estava preso ou morreu)
        if (player.getPosicao() == this.posicaoAntesDoTurno) {
            return null;
        }

        int posicao = player.getPosicao();
        if (!tabuleiro.containsKey(posicao)) { return null; }
